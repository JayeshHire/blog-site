...
const optionsFunctionality = () => {
    ...
    optionIdTextMp.set('op-logout', 
        'this is the option for logging out. this will show a short form for logging out'
    ) ;
    optionIdTextMp.set('op-signup', 
        'this is the option for signing up. This is for new user. This will open a new form for users to enter information and signin.'
    ) ;
    optionIdTextMp.set('op-login',
        'this is the option for logging in to the application for already existing users.'
    )
    ...
    for (const [ele_id, text] of optionIdTextMp){
        ...
        ele.addEventListener("click", ()=> {
            opContentWin.style.visibility = "visible" ;
            opContentWinChild.innerText = text ;
        }) ;
    }
}
